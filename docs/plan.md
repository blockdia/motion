...
状态：P0、P1a 编辑器布局基线、P1b 通用教程闭环和 P1c Target/完整 toolbox 已完成（2026-09-12），见 [P0 验证记录](p0.md)、[P1a 基线](p1a.md) 和 [P1b 验收与支持矩阵](p1b.md)。P1c 已完成真实目录提取与 target 隔离，见 [P1c 验收记录](p1c.md)；P2 常规代码编辑已实现，见 [P2 支持矩阵与验收](p2.md)；P3 已完成浏览器有限交互与视觉变体，见 [P3 验收](p3.md)；P4 导出质量与性能已完成，见 [P4 基准与验收](p4.md)；P5 仍为后续计划。
...
- UI 状态图层截图缓存、SVG 一次栅格化缓存、RGBA 帧合成、有限缓存、帧流背压、取消和失败清理。
...
P2 已补齐拆分、删除、嵌套输入、字段菜单、模拟输入法与标注，具体契约和限制见 [P2](p2.md)。P3、P4 已完成。当前默认导出使用 Chrome UI 图层缓存、resvg 素材缓存、RGBA 帧合成与 FFmpeg；整场 Chrome 截图保留为参考后端。有界缓存、并发、取消、量化预算和重复长教程基准见 [P4](p4.md)。P0/P1 的 resvg 数据保留为历史成本参考。
