...
状态：P0 固定示例验证已完成（2026-09-12），实现与测量见 [P0 验证记录](p0.md)。P1 及后续内容仍为实施计划，不代表已有实现。
...
验收：示例教程无作者坐标、无编辑器 selector；随机时间求值与顺序播放一致；视频帧数与时长正确；缺失资源/目标有明确诊断。
...
下一步是 P1 的可求值端到端最小版本：将 P0 的固定示例拆出场景协议、语义编译与作者接口，复用完整栈/子树素材及 resvg + FFmpeg 路线。P0 的无缓存基线用于识别成本，尚不作为生产性能或并发预算。
