...
状态：P0 固定示例验证与 P1a 编辑器布局基线已完成（2026-09-12），见 [P0 验证记录](p0.md) 和 [P1a 基线](p1a.md)。P1b 及后续内容仍为实施计划，不代表已有实现。
...
- Scratch VM 运行、项目真实执行结果、完整的交互式 IDE。
...
1. **基础目录**：常用积木和 toolbox 分类的默认视觉，可预先生成。
...
按分类、语言和教程使用范围准备 toolbox，纯颜色主题共用结构。分类栏由 UI 层绘制，分类内容以独立的积木资源和布局元数据表示，便于滚动、命中、提取拖动副本。

从 toolbox 拖出后，源条目保留；复制目录条目内容并分配作者指定的工作区逻辑积木 ID。工具箱实例 ID 与工作区实例 ID 分开。首期只显示教程允许的分类和条目，不承诺动态扩展加载。
...
- **随后完成 P1b：通用教程闭环。** 以下核心协议与编排工作建立在 P1a 的布局上，避免后期重做定位与裁剪。
...
下一步是 P1b 的可求值端到端最小版本，建立在已完成的 P1a 布局基线上：将 P0 的固定示例拆出场景协议、语义编译与作者接口，复用完整栈/子树素材及 resvg + FFmpeg 路线。P0 的无缓存基线用于识别成本，尚不作为生产性能或并发预算。
