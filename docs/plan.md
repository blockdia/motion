...
状态：P0、P1a 编辑器布局基线、P1b 通用教程闭环和 P1c Target/完整 toolbox 已完成（2026-09-12），见 [P0 验证记录](p0.md)、[P1a 基线](p1a.md) 和 [P1b 验收与支持矩阵](p1b.md)。P1c 已完成真实目录提取与 target 隔离，见 [P1c 验收记录](p1c.md)；P2 常规代码编辑已实现，见 [P2 支持矩阵与验收](p2.md)；P3 及后续仍为实施计划。
...
P2 已补齐拆分、删除、嵌套输入、字段菜单、模拟输入法与标注，具体契约和限制见 [P2](p2.md)。下一步进入 P3 浏览器有限交互与视觉变体。P1b 继续复用 resvg + FFmpeg 路线；当前无静态栅格缓存的数据用于识别成本，尚不作为生产性能或并发预算。
