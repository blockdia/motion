...
编译产物的 `finalTargets` 保存分 target 的最终积木图，`finalBlocks` 提供全部根；manifest 的目录只位于 `targets`，没有另一个单角色目录。旧 P1b spec 需补充这两个项目字段；仓库的 TypeScript/JSON 示例已同步，旧编译文件需要重新编译。
...
条目键为 `分类.opcode.完整规范化定义的SHA256前16位`。相同 opcode 的不同变量、参数或 mutation 使用不同键；完全相同的重复定义按原目录出现次序追加序号。逻辑子 ID 使用结构路径，不使用 Blockly 的随机实例 ID。三个旧公开键仅作为真实条目的别名，不携带手写默认内容。
...
| 文本输入           | 实际文本字段；保留 `steps`、`message` 语义别名；菜单、颜色和其他字段编辑报 `CAPABILITY` |
...
`scripts/p1c.mjs` 在 workspace 释放后生成同样区域及切换恢复后的 10 组浏览器/resvg 关键帧，按 P1b 阈值检查一致性，并导出 4.6 秒 / 138 帧 / 1280×720 / 30 fps 视频、核对 ffprobe 帧数，见 [report.json](p1c-baseline/report.json)。本次 10 组双端图像的最大平均通道误差为 0.516，超过 32 的差异像素比例最高为 0.761%，低于 P1b 的验收阈值。`scripts/p1b.mjs` 同时回归原教程、CLI、输入动画、播放控制、随机跳转及视频导出。

P1a 的顶栏、标签页、区域比例和留白保持不变。允许差异仍包括空白舞台与简化角色区；新增当前 target 名称、受控按钮/复选框绘制、确定性的分类滚动时长。真实 GUI 的参考项目含过程定义与保留广播的种子脚本，只用于验证目录上下文，不属于 Motion 教程的初始工作区。
