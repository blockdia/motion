...
主要区域沿用 P1a 的 layout/chrome。P1c 已取消旧的教程条目过滤和单分类页面：完整目录连续滚动，分类标题、标签、按钮、复选框与积木位置来自真实 flyout；分类栏标签来自实际语言结果，滚动条跟随内容范围。舞台和角色画面仍留空，当前 target 名称可见。详细对照与限制见 [P1c](p1c.md)。
