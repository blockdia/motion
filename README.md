...
已完成 **P1a 编辑器布局与样式基线**，包含固定版本 TurboWarp 的真实参考截图、受控外壳与双端关键帧；保留 **P0 固定示例验证**：从工具箱拖出事件帽和移动积木、连接、将参数 10 改为 20；包含真实 Blockly SVG 提取、中文与嵌套样例、独立主题样式、无 Blockly 的播放器，以及 resvg + FFmpeg 视频导出。
...
- [P1a 参考画面、布局、允许差异与验证](docs/p1a.md)
- [P0 复现、来源、边界与性能基线](docs/p0.md)
- [第三方素材来源](THIRD_PARTY_NOTICES.md)

通用 TypeScript/JSON 作者 API 和编译器属于后续 P1b。
