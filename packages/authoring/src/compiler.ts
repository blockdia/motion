...
export async function compilePrepared(
  input: unknown,
  adapter: PreparationAdapter,
  typing: Record<string, import('@blockdia-motion/core').TypingFrame[]>,
  ...
  async function run(s: Step, c: Context, t: number, path: string): Promise<number> {
    ...
    if (s.op === 'setField' || s.op === 'choose') {
      ...
      if (s.op === 'choose') {
        ...
        const above = bounds.y + bounds.height + height + 20 > view.y + view.height;
        const panel = {
          x: Math.max(
            view.x + 8,
            Math.min(at.x - menu.width / 2, view.x + view.width - menu.width - 8),
          ),
          y: above ? Math.max(view.y + 8, bounds.y - height - 20) : bounds.y + bounds.height + 20,
          width: menu.width,
          height,
        };
    ...
    if (s.op === 'type') {
      ...
      const planned = typing[s.value];
      if (!planned) fail('RESOURCE', path, 'Missing typing sequence');
  ...
  const result: CompiledScene = {
    schemaVersion: 1,
    duration: Math.max(duration, ...(spec.stage?.clips ?? []).map((c) => c.start + c.duration)),
