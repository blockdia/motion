...
const reference = referenceArg >= 0 ? process.argv[referenceArg + 1] : undefined;
if (referenceArg >= 0 && (!reference || reference.startsWith('--')))
  throw Error('--reference requires a saved exporter module path');
const engines = reference ? ['javascript', 'skia'] : ['screenshot', 'composite'];
const exporters = reference
  ? {
      javascript: (await import(pathToFileURL(resolve(reference)).href)).exportVideo,
      skia: exportVideo,
    }
  : { screenshot: exportVideo, composite: exportVideo };
const out = reference ? 'artifacts/p4-native' : 'artifacts/p4-comparison';
        ...
        reference: reference
          ? {
              ...
              sources: Object.fromEntries(
                await Promise.all(
                  ['index.js', 'compositor.js', 'bitmap.js', 'cache.js', 'memory.js'].map(
                    async (p) => [p, hash(await readFile(resolve(dirname(reference), p)))],
                  ),
                ),
              ),
            }
...
for (const { name, spec, ...options } of cases) {
  ...
  for (const backend of engines) {
    ...
    const result = await exporters[backend](bundle, {
      ...options,
