...
test('all API tutorial prepares in the consuming playground; persistent nodes, controls, and disposal', async () => {
  ...
  try {
    ...
    await frame.evaluate(() => window.player.seek(2));
    const stable = await frame.evaluate(async () => {
    ...
    for (const t of [0, 1.5, 5, 8, 12, 16, 20, 25, 26.8, 2])
      await frame.evaluate((t) => window.player.seek(t), t);
    const boundary = await frame.evaluate(async () => {
    ...
    await frame.locator('.motion-frame').focus();
    await h.page.keyboard.press('ArrowLeft');
    assert.equal(await frame.evaluate(() => window.player.view.x), 30);
    await h.page.keyboard.press('0');
    assert.equal(await frame.evaluate(() => window.player.view.x), 0);
    await frame.evaluate(() => {
      window.player.setCursorClickEffect('shrink');
      window.player.setCursorMotion('curve');
    });
    assert.equal(await frame.evaluate(() => window.player.cursorMotion), 'curve');
    for (const width of [375, 800, 1280]) {
      await h.page.setViewportSize({ width, height: 900 });
      await frame.evaluate(() => window.player.seek(4));
    ...
    await frame.evaluate(() => window.player.dispose());
    assert.equal(await frame.locator('#player > *').count(), 0);
